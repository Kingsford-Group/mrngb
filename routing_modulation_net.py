...
class MetaModulationLearner(nn.Module):
    def __init__(self, c, h, w, ways, shots, queries,
        ...
        self.routers = METHOD_ROUTERS[method_name] if method_name in METHOD_ROUTERS.keys() else ['id','id','id','id']
        ...
        if not resnet:
            ...
            self.predictor_net = ResNetRouting(ways, self.routers)
        if method_name in ['mrn', 'mrngs']:
            router_depth = 0
        else:
            router_depth = int(method_name[-1]) - 1
        switch_dim = torch.numel(self.predictor_net.router[router_depth].params['states'])
    ...
    def meta_train(self, task, tasksets, max_epoch=800, meta_batch_size=32, test_batch_size=5, l=1.0):
        ...
        maml = l2l.algorithms.MAML(
            self.predictor_net, lr=5e-3 if self.resnet else 5e-1,
            first_order=False,
            allow_unused=True,
            allow_nograd=True
        )
...
def run_mrn(task, max_epoch=12000, meta_batch_size=32, test_batch_size=5):
    seed()
    cfg = config[task]
    cfg['method_name'] = 'mrn'
...
def run_mrn_resnet(task, max_epoch=2000, meta_batch_size=32, test_batch_size=5):
    seed()
    cfg = config[task]
    cfg['method_name'] = 'mrn'
    cfg['resnet'] = True
...
def run_mrn_depth(task, max_epoch=200, meta_batch_size=32, test_batch_size=5):
    seed()
    cfg = config[task]
    base_method_name = 'mrn'
    for d in [2, 3 ,4]:
        cfg['method_name'] = base_method_name + f'd{d}'
        cfg['save_dir'] = cfg['save_dir'] + f'-d{d}'
        model = MetaModulationLearner(**cfg)
        model = model.to(device)
        tasksets = TASKS[task](model.ways, model.shots, model.queries)
        results = model.meta_train(task, tasksets, max_epoch, meta_batch_size, test_batch_size)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        # model, results = run_mrn_depth('omniglot')
        # # model, results = run_mrn_resnet('jigsaw-mini-imagenet-hard')
        model, results = run_mrn('omniglot')
